...
public class BlockChain {
    ...
    public boolean addBlock(Block block) {
    	...
    	List<Node> branch = getNodeBranch(GN, 
    	...
    	if (!isTransactionsValid(block, parent)) {
    		return false;
    	}    	
	...
	private boolean isTransactionsValid(Block block, Node parent) {		
	...
	private List<Node> getNodeBranch(Node node, ByteArrayWrapper blockHash) {    	
    	...
    	for (Node child : node.children) {
    		branch = getNodeBranch(child, blockHash);
    		if (branch.size() > 0) {
    			branch.add(child);
    			return branch;
    		}
